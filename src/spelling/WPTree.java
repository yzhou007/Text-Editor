...
public class WPTree implements WordPath {
	...
	{	
		...
		visited.add(word1);
		queue.add(root);
		while(!queue.isEmpty() && !root.buildPathToRoot().contains(word2) && count < maxIte){
			count++;
			...
			for(String st:l){
				if(!visited.contains(st)){
					curr.addChild(st);
					visited.add(st);
					queue.add(curr.getChildren().get(curr.getChildren().size()-1));
					if (st.equals(word2)){
						retList=curr.getChildren().get(curr.getChildren().size()-1).buildPathToRoot();
