...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
		...
		{	
			...
			BasicDocument docBas = new BasicDocument(text);
			docBas.getFleschScore();
			}
			long endTimeBas = System.nanoTime();
			
			double timeBas = (endTimeBas-startTimeBas)/1.0e9;

			
			
			long startTimeEff = System.nanoTime();
			for (int i=0; i<trials; i++){
			EfficientDocument docEff = new EfficientDocument(text);
			docEff.getFleschScore();
			}
			long endTimeEff = System.nanoTime();
			
			double timeEff = (endTimeEff-startTimeEff)/1.0e9;
			
			if (numToCheck == start){
				System.out.println("Size" + "\t" + "BasicDocument" + "\t" + "EfficientDocument");
			}
			/*System.out.print(numToCheck + "\t");
			System.out.print(timeBas + "\t"); 
			System.out.print(timeEff + "\n");*/
			
			System.out.println(numToCheck + "\t" + timeBas + "\t" + timeEff);
